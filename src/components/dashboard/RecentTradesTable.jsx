              ...
              >
                <td className="px-6 py-5 rounded-l-2xl">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-bg-darker flex items-center justify-center text-lg border border-white/[0.05] group-hover/row:border-primary/20 transition-colors">
                      {trade.icon}
                    </div>
                    <div>
                        <p className="text-sm font-black text-white tracking-tight">{trade.asset}</p>
                        <p className="text-[10px] font-medium text-text-muted">{trade.strategy}</p>
                    </div>
                  </div>
                </td>
                ...
                <td className="px-6 py-5 text-sm font-black text-white/90 tracking-tight">{trade.rMultiple}</td>
                <td className={`px-6 py-5 text-sm font-black ${trade.result === 'Win' ? 'text-trading-green' : 'text-trading-red-glow'}`}>
                  {trade.pnl}
                </td>
                <td className="px-6 py-5 text-right rounded-r-2xl">
                    <div className="flex items-center justify-end gap-4">
                        ...
                        <MoreHorizontal className="w-4 h-4 text-text-muted opacity-0 group-hover/row:opacity-100 transition-opacity" />
                    </div>
                </td>
                
                {/* Visual accent for active row */}
                <div className={`absolute inset-y-0 left-0 w-1 ${trade.result === 'Win' ? 'bg-trading-green' : 'bg-trading-red'} rounded-l-2xl scale-y-0 group-hover/row:scale-y-100 transition-transform duration-300`} />
              </motion.tr>
            ))}
          </tbody>
        </table>
